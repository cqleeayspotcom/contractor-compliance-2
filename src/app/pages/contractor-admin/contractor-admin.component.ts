...
export class ContractorAdminComponent implements OnInit, OnDestroy {
  ...
  private loadFailedTasks(): void {
    // Pas de queue de tasks générique exposée côté module ContractorCompliance
    // (les retries OCR/Stripe passent par circuit-breakers + webhook DLQ
    // observables via /admin/circuit-breakers et /admin/webhooks/logs).
    // On expose donc une liste vide — l'UI affiche déjà "rien à retenter".
    this.failedTasksList.set([]);
  }

  private loadQueues(): void {
    // Pas de queues observables côté module (RabbitMQ shell-queue est global
    // Tuita, hors périmètre compliance). On expose des compteurs neutres.
    this.queueStatus.set({ pending: 0, processing: 0, failed: 0 });
  }
  ...
  retryFailedTask(_taskId: string | number): void {
    // Pas de retry générique côté backend Tuita : les retries passent par
    // les actions ciblées (replay webhook, reset circuit-breaker, retry
    // purchase) qui ont chacune leur bouton dédié. Rafraîchissement pour
    // garder la cohérence visuelle de l'UI.
    this.loadQueues();
    this.loadFailedTasks();
  }
